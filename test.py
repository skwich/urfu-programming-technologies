...
def sort_salary(text):
    p = re.findall(r'\d+\s\d+', text)[1].replace(' ', '')
    print(p)
    return int(p)

def sort_exp(text):
    p = re.findall(r'\d', text)
    return int(p) if len(p) else 0

def sort_data(text):
    return datetime.striptime(text, '%H:%M:%S %d/%m/%Y') #17:35:20 27/06/2022
...
def apply_sort(func):
    def wrapper(*args):
        ...
        if sort_column == '':
            ...
            vacancies.sort(reverse=is_sort_reverse, key=lambda d: sort_salary(d[sort_column]))
        elif sort_column == "Опыт работы":
            vacancies.sort(reverse=is_sort_reverse, key=lambda d: sort_exp(d[sort_column]))
        elif sort_column == "Дата публикации вакансии":
            vacancies.sort(reverse=is_sort_reverse, key=lambda d: sort_data(d[sort_column]))
        return vacancies
    return wrapper
...
def main():
    file_name = '6.Функциональное программирование/vacancies_for_functional.csv'

    filter_str = 'Идентификатор валюты оклада: Рубли'
